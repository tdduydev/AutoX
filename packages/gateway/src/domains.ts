...
const installedDomainIds = new Set<string>(['general']);

export function getInstalledDomainIds(): Set<string> {
  return installedDomainIds;
}

export function createDomainRoutes(domainPacks: DomainPack[]) {
  const app = new Hono();

  const findDomain = (id: string) => domainPacks.find((d) => d.id === id);

...
export class ImageGenService {
  ...
  listModels() {
    return IMAGE_MODELS[this.config.provider as keyof typeof IMAGE_MODELS] || [];
  }

  // ─── Gemini (Google Imagen) ──────────────────────────────

  private async generateGemini(req: ImageGenRequest, count: number, start: number): Promise<ImageGenResult> {
    ...
    if (!data.predictions?.length) throw new Error('Gemini Imagen: no images returned');
  ...
  private async generateReplicate(req: ImageGenRequest, count: number, start: number): Promise<ImageGenResult> {
    const apiKey = this.config.apiKey;
    if (!apiKey) throw new Error('REPLICATE_API_KEY is required');
    ...
    for (let i = 0; i < count; i++) {
      ...
      const prediction = await createRes.json() as { output?: string | string[]; status?: string; error?: string };

      if (prediction.error) throw new Error(`Replicate: ${prediction.error}`);

      const outputUrl = Array.isArray(prediction.output) ? prediction.output[0] : prediction.output;
      if (!outputUrl) throw new Error('Replicate: no output returned');
  ...
  private async generateTogether(req: ImageGenRequest, count: number, start: number): Promise<ImageGenResult> {
    const apiKey = this.config.apiKey;
  ...
  private async generateComfyUI(req: ImageGenRequest, count: number, start: number): Promise<ImageGenResult> {
  ...
  private buildComfyWorkflow(req: ImageGenRequest, _model: string, index: number): Record<string, unknown> {
  ...
  private async pollComfyResult(baseUrl: string, promptId: string, maxWait = 120000): Promise<string> {
  ...
  private async generatePlaceholder(req: ImageGenRequest, count: number, start: number): Promise<ImageGenResult> {

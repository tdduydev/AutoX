...
function normalizeNode(raw: any): WorkflowNode {
  ...
  if (raw.data && typeof raw.data.label === 'string') return raw;
  return {
    id: raw.id,
    type: raw.type,
...
function normalizeEdge(raw: any): WorkflowEdge {
  ...
  if (typeof raw.source === 'string' && typeof raw.target === 'string') return { id: raw.id ?? randomUUID(), sourcePort: raw.sourcePort ?? 'default', targetPort: raw.targetPort ?? 'default', ...raw };
  return {
...
function buildWorkflow(row: any): Workflow {
...
export function createWorkflowRoutes(workflowEngine: WorkflowEngine) {
    ...
    try {
      ...
      return c.json({
        ...
        workflows: rows.map(r => ({
          ...
          version: r.version,
          enabled: r.enabled,
          nodeCount: (r.definition as any)?.nodes?.length ?? 0,
  ...
  app.post('/', async (c) => {
    try {
      ...
      const body = await c.req.json<{
        name: string;
        description?: string;
        definition: { nodes: any[]; edges: any[]; variables?: any[]; trigger?: any };
      }>();
  ...
  app.post('/:id/validate', async (c) => {
    try {
      ...
      return c.json({ ok: true, valid: errors.filter((e: ValidationError) => e.severity === 'error').length === 0, errors });
    } catch (err) {
      return c.json({ error: err instanceof Error ? err.message : 'Failed' }, 500);
    }
  });
  ...
  app.post('/:id/execute', async (c) => {
    try {
      ...
      const body = await c.req.json<{ triggerData?: Record<string, unknown> }>().catch(() => ({ triggerData: undefined }));

      const workflow = buildWorkflow(row);

      // Validate before executing
      const validationErrors = workflowEngine.validate(workflow);
      const hasErrors = validationErrors.filter((e: ValidationError) => e.severity === 'error').length > 0;
      if (hasErrors) {

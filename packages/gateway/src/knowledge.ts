...
export function createKnowledgeRoutes(ctx: GatewayContext) {
  const app = new Hono();
  ...
  app.delete('/collections/:id', (c) => {
    ...
    if (!ok) return c.json({ error: 'Cannot delete default collection' }, 400);
  ...
  app.post('/upload', async (c) => {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      source = (form.get('source') as string) || 'upload';
      const tagsStr = form.get('tags') as string;
      if (tagsStr) tags = tagsStr.split(',').map((t) => t.trim()).filter(Boolean);
      collectionId = (form.get('collectionId') as string) || undefined;
      const metaStr = form.get('customMetadata') as string;
      if (metaStr) try { customMetadata = JSON.parse(metaStr); } catch {}
      const cs = form.get('chunkSize') as string;
      if (cs) chunkSize = parseInt(cs, 10);
      const co = form.get('chunkOverlap') as string;
      if (co) chunkOverlap = parseInt(co, 10);
  ...
  app.get('/:id/chunks', (c) => {
    ...
    if (!chunks) return c.json({ error: 'Document not found' }, 404);
  ...
  app.post('/:id/chunks', async (c) => {
    ...
    if (!chunk) return c.json({ error: 'Document not found' }, 404);
  ...
  app.put('/:id/chunks/:chunkId', async (c) => {
    ...
    if (!ok) return c.json({ error: 'Chunk not found' }, 404);
    return c.json({ success: true });
  });
  ...
  app.post('/batch/delete', async (c) => {
    ...
    if (!Array.isArray(documentIds)) return c.json({ error: 'documentIds array required' }, 400);
  ...
  app.post('/batch/move', async (c) => {
    ...
    if (!Array.isArray(documentIds) || !collectionId) return c.json({ error: 'documentIds and collectionId required' }, 400);
  ...
  app.post('/refresh/:id', async (c) => {
    try {
      ...
      if (!doc) return c.json({ error: 'Document not found or not web-sourced' }, 404);

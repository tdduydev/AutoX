...
export interface OAuth2ProviderConfig {
  ...
  userInfoUrl: string;
  scopes: string[];
  // Maps provider profile to our user fields
...
const PROVIDER_CONFIGS: Record<string, Omit<OAuth2ProviderConfig, 'clientId' | 'clientSecret'>> = {
  google: {
...
function getProviderConfig(provider: string): OAuth2ProviderConfig | null {
...
async function fetchUserProfile(config: OAuth2ProviderConfig, accessToken: string): Promise<Record<string, any>> {
...
export function createOAuth2Routes(ctx: GatewayContext) {
  const app = new Hono();
  ...
  app.get('/providers', (c) => {
    ...
    for (const provider of Object.keys(PROVIDER_CONFIGS)) {
      if (getProviderConfig(provider)) enabled.push(provider);
    }
  ...
  app.get('/:provider/callback', async (c) => {
    ...
    const [existingOAuth] = await db.select().from(oauthAccounts)
      .where(and(
        ...
        eq(oauthAccounts.providerAccountId, mapped.providerAccountId),
        eq(oauthAccounts.tenantId, tenant.id),
      ))
      .limit(1);
  ...
  app.post('/:provider/link', authMiddleware(ctx.config.jwtSecret), async (c) => {
    ...
    const [existing] = await db.select().from(oauthAccounts)
      .where(and(
        eq(oauthAccounts.provider, provider),
        eq(oauthAccounts.providerAccountId, mapped.providerAccountId),
        eq(oauthAccounts.tenantId, currentUser.tenantId),
      ))
      .limit(1);
  ...
  app.delete('/:provider/link', authMiddleware(ctx.config.jwtSecret), async (c) => {
    ...
    await db.delete(oauthAccounts)
      .where(and(
        eq(oauthAccounts.userId, currentUser.sub),
        eq(oauthAccounts.provider, provider),
      ));

...
export function createMLRoutes(engine: MLEngine) {
  ...
  ml.post('/datasets/upload', async (c) => {
    const body = await c.req.json<{ data: string; format: 'csv' | 'json'; name?: string }>();
  ...
  ml.get('/datasets/:id/profile', (c) => {
    ...
    if (!profile) return c.json({ error: 'Dataset not found' }, 404);
  ...
  ml.post('/train', async (c) => {
    ...
    if (!model) return c.json({ error: 'Training failed — check dataset/algorithm IDs' }, 400);
  ...
  ml.post('/models/compare', async (c) => {
    ...
    if (!body.modelIds?.length) return c.json({ error: 'modelIds required' }, 400);

...
export const ALL_PERMISSIONS: Array<{ resource: Resource; action: Action; description: string }> = [];
...
export async function assignRoleToUser(userId: string, roleName: string, tenantId: string, assignedBy?: string) {
  const db = getDB();

  const [role] = await db.select().from(roles)
    .where(and(eq(roles.tenantId, tenantId), eq(roles.name, roleName)))
    .limit(1);
  if (!role) throw new Error(`Role '${roleName}' not found for tenant`);
...
export function createRBACRoutes() {
  ...
  app.put('/roles/:roleId', requirePermission('roles', 'manage'), async (c) => {
    ...
    if (role.isSystem) return c.json({ error: 'Cannot modify system roles' }, 403);
  ...
  app.delete('/roles/:roleId', requirePermission('roles', 'manage'), async (c) => {
    ...
    const [role] = await db.select().from(roles)
      ...
      .limit(1);
    if (!role) return c.json({ error: 'Role not found' }, 404);
    if (role.isSystem) return c.json({ error: 'Cannot delete system roles' }, 403);
